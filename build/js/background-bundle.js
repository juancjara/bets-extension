...
var calculateResults = function calculateResults(game) {
  game.results = game.GameResults[0].GameResultValue + '-' + game.GameResults[1].GameResultValue;
  return game;
};
...
function parseData(data, skips) {
  ...
  //need to iterate over liveEvents , over each game,
  ev.events.filter(gameWithResults).map(calcutateResults);
  ...
  var niceData = liveEvents.map(function (ev) {
    return {
      ...
      events: ev.events.map(function (game) {
        ...
        odds.sort().reverse();
        if (!skips[game.eventId] && shouldNotify) {
          if (true) {}
        }
        if (!skips[game.EventId] && (shouldNotify || shouldNotify2 && odds[0] >= 2.0 && odds[1] >= 2.0)) {
...
var notify = function notify(eventName, results) {
  _chromeNotification2['default'].create(eventName + ', result: ' + results);
};
...
},{"./alarm":4,"./chromeNotification":6,"./db":7,"./httpRequest":8}],6:[function(require,module,exports){
'use strict';
...
},{"localforage":1,"moment":2}],8:[function(require,module,exports){
"use strict";

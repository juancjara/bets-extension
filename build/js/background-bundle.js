...
var calculateGameTime = function calculateGameTime(game) {
  game.elapsedTime = _lodash2['default'].floor(game.MatchClockSeconds / 60);
  return game;
};

var calculateResults = function calculateResults(game) {
  game.results = [Number(game.GameResults[0].GameResultValue), Number(game.GameResults[1].GameResultValue)];
  return game;
};

var extractOdds = function extractOdds(game) {
  var teamsOdds = [];
...
var extractViewData = function extractViewData(game) {
  return {
    id: game.EventId,
...
var goalsDiffGreaterEqualThan = function goalsDiffGreaterEqualThan(results, diff) {
...
var launchNotifications = function launchNotifications(events, skips) {
  _lodash2['default'].flatten(_lodash2['default'].pluck(events, 'games')).filter(function (game) {
    return !skips[game.id];
...
},{"localforage":1,"moment":3}],9:[function(require,module,exports){
"use strict";
